...
utility classes, many of which build on the WizardWrx .NET API
...
This library is fully documented at https://txwizard.github.io/ConsoleAppAids3,
and the WizardWrx .NET API is fully documented online at https://txwizard.github.io/WizardWrx_NET_API.
...
*	DisplayAids is a sealed (implicitly static) class that precisely
controls the way your application handles pauses.

*	FixedConsoleWriter permits a line of a console window to be used
...
Note: I have not tested this library with .NET Core.
...
you incorporate this library, all but one of the WizardWrx .NET API
...
Note: This section is applicable only to the source code repository. The NuGet
...
AssemblyVersion tracks the build numbers as we go, and changes with every
...
AssemblyFileVersion ties together all the assemblies in this API. This is
...
1.	A NuGet Package arrives with this release.
...
1.	Naming Conventions: I use Hungarian notation. Some claim that it has
...
2.	Coding Style: I have my editor set to leave spaces around every token.
...
3.	Comments: I comment liberally and very deliberately. Of particular
...
document which of two paths an if statement is expected to follow most of
the time. When blocks get nested two, three, or four deep, they really earn
their keep.

4.	Negative Conditions: I do my best to avoid them, because they almost
...
5.	Array Initializers: Arrays that have more than a very few initializers,
...
6.	Format Item Lists: Lists of items that are paired with format items in
...
7.	Symbolic Constants: I use symbolic constants to document what a literal
...
8.	Argument Lists: I treat argument lists as arrays, and often comment each
...
9.	Triple-slash Comments: These go on _everything_, even private members and
